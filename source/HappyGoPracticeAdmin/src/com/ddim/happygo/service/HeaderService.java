...
import com.ddim.happygo.dao.HeaderDao;
...
public class HeaderService {
	...
	public Page<Header> findPage(String name, String status, PagingParameter paging) throws Exception {
		return headerDao.findPage(name, status, paging);
	}
	...
	public void save(Header entity, String modifier) throws Exception {
		...
		if (StringUtils.isBlank(entity.getId())) {
			...
			int dataNums = headerDao.countList();
			entity.setSort(dataNums+1);
			headerDao.insert(entity);
		} else {
			entity.setModifier(modifier);
			entity.setModify_time(now);
	...
	public void delete(String[] ids, String modifier) throws Exception {
		for (String id : ids) {
			Header entity = get(id);
			headerDao.delete(entity.getSort(),entity.getId());
		}
	}
	 ...
	 */
	@Transactional(rollbackFor = Exception.class)
	public void updateOpenMethod(String[] ids, String target, String modifier) throws Exception {
		for (String id : ids) {
			Header entity = get(id);
			headerDao.updateOpenMethod(entity.getId(), target, modifier);
		}
	}
	...
	public void updateStatus(String[] ids, String status, String modifier) throws Exception {
		for (String id : ids) {
			Header entity = get(id);
			headerDao.updateStatus(entity.getId(), status, modifier);
		}
	}
	...
	public void updateSort(String id, int sort, String modifier) throws Exception {
		Header entity = get(id);
		headerDao.updateSort(id, entity.getSort(), sort, modifier);
	}
