...
import com.ddim.happygo.dao.LongAdsDao;
 ...
 */
@Service
public class LongAdsService {

	@Autowired
	private LongAdsDao longAdsDao;
	...
	public Page<LongAds> findPage(String name, String status, PagingParameter paging) throws Exception {
		return longAdsDao.findPage(name, status, paging);
	}
	...
	public void save(LongAds entity, String modifier) throws Exception {
		...
		if (StringUtils.isBlank(entity.getId())) {
			...
			int dataNums = longAdsDao.countList();
			entity.setSort(dataNums+1);
			longAdsDao.insert(entity);
		} else {
			entity.setModifier(modifier);
			entity.setModify_time(now);
			longAdsDao.update(entity);
		}
	}
	...
	public void delete(String[] ids, String modifier) throws Exception {
		for (String id : ids) {
			LongAds entity = get(id);
			longAdsDao.delete(entity.getSort(),entity.getId());
		}
	}
	...
	public void updateStatus(String[] ids, String status, String modifier) throws Exception {
		for (String id : ids) {
			LongAds entity = get(id);
			longAdsDao.updateStatus(entity.getId(), status, modifier);
		}
	}
	...
	public void updateSort(String id, int sort, String modifier) throws Exception {
		LongAds entity = get(id);
		longAdsDao.updateSort(id, entity.getSort(), sort, modifier);
	}
