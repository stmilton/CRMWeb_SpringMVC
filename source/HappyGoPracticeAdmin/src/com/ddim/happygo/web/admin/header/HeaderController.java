...
//	private String fileReadUrl;
	...
	protected String getRightID() {
		return Constants.HEADER_TASK_KEY;
	}
	...
	private void setCommonData(HttpServletRequest request) throws Exception {
		...
		request.setAttribute("countList", headerService.countList());
	}
	...
	public String list(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, PagingParameter paging,
		...
		try {
			...
			Page<Header> page = headerService.findPage(form.getName(), form.getStatus(), paging);
	...
	public String edit(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, ModelMap model)
		...
		try {
			...
			if (StringUtils.isNotBlank(form.getId())) {
				Header data = headerService.get(form.getId());
				if (data == null) {
	...
	public String save(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, BindingResult result,
		...
		try {
			...
			formValidator.validate(form, result, fileUploadPath + headerPath);
			if (result.hasErrors()) {
				setCommonData(request);
				return getPath() + "/edit";
			}
			...
			headerService.save(entity, managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
					"save header success: " + entity.getId());
	...
	public String update(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, ModelMap model)
		...
		try {
			...
			headerService.updateStatus(form.getIds(), form.getStatus(), managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
					"update header status success: " + form.getIds());
			model.addAttribute("message", MessageConstants.MESSAGE_SAVE_SUCCESS);
			model.addAttribute("relativeUrl", "list.do");
			return "common/jsAlert";
		} catch (Exception e) {
	...
	public String delete(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, ModelMap model)
		...
		try {
			...
			headerService.delete(form.getIds(), managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
					"delete header success: " + form.getIds());
	...
	public String updateSort(HttpServletRequest request, @ModelAttribute("command") HeaderForm form, ModelMap model)
			throws Exception {
		try {
			String returnPath = managerUtil.haveRight(request, getRightID());
			if (returnPath != null) {
			...
			headerService.updateSort(form.getId(), form.getSort(), managerUtil.getManagerAccount(request));
			LogUtil.setSaveLog(managerUtil.getManagerAccount(request),
					"update header sort success: " + form.getIds());
