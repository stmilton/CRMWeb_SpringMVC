 ...
 * 類別名稱：FooterFormValidator.java<P>
 * 程式內容說明：最新消息資料 驗證<P>
 * @author Yvonne
 * */
@Component
public class FooterFormValidator {
	 ...
	 * @throws Exception
	 */
	public void validate(FooterForm form, BindingResult result, String uploadFilePath) throws Exception {
		if (StringUtils.isBlank(form.getName())) {
			result.rejectValue("name", "error", "請輸入");
		}
		if(form.getName().length() > 20) {
			result.rejectValue("name", "error", "地功能內容名稱過長");
		}
