	...
	<form:form method="post" id="sForm" name="sForm" action="save.do"
		...
		<table class="tbType" cellspacing="0" cellpadding="0" border="0"
			...
			<tbody>
				...
				<tr>
					<th scope="row"><span class="red">※</span>大Banner圖片(PC用)<br />(圖片尺寸
						720 * 400)</th>
					<td><form:hidden path="image" /> 
						
						<c:if
							test="${not empty command.image}">
							<img src="<c:out value="${imagePath}"/><c:out value="${command.image}"/>" width="720" height=400""/>
						</c:if> 
						<br/>
				...
				<tr class="bg">
					<th scope="row"><span class="red">※</span>廣告文案</th>
					<td><form:textarea path="ad_proposal" rows="10" cols="20"
							style="margin: 0px; width: 400px; height: 300px;" /> <form:errors
							path="ad_proposal" cssClass="error" /></td>
				</tr>
...
<script>
	var tinymce_selector = "#description";
</script>
<script type="text/javascript"
