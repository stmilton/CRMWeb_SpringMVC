	...
	<form:form method="post" id="sForm" name="sForm" action="save.do" enctype="multipart/form-data">
		...
		<table class="tbType" cellspacing="0" cellpadding="0" border="0" width="100%">
			<tbody>
				...
				<tr>
					<th scope="row"><span class="red">※</span>圖片(PC用)<br/>(圖片尺寸 380*380)</th>
					<td>
						<form:hidden path="image"/>
						<c:if test="${not empty command.image}">
							<img src="<c:out value="${imagePath}"/><c:out value="${command.image}"/>" width="380" height="380"/>
						</c:if>
						<br/>
				...
				<tr class="bg">
					...
					<td><form:textarea path="ad_proposal" rows="10" cols="20" style="margin: 0px; width: 400px; height: 300px;"/> 
